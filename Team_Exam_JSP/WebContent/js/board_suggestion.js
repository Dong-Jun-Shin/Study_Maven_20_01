function template(no, best, subject, writer, hit, date){
			var $tbody = $('div.example > table > tbody#board_list');
			
			var $element = $('#suggestion-template').clone().removeAttr('id');
		...
		$(function(){
			...
			$.get("../json/board_suggestion.json", {}, function(json){
				...
				for (var i = 0; i < item.length; i++){
					template(item[i].no, item[i].best, item[i].subject, item[i].writer, item[i].hit, item[i].date);
				}
			});
		});	//$ 종료구문